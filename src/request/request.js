...
function requestApi ({ name, data, header, headerType }) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (next) {
      const res = await Request(options, data, header, hooks, headerType)
      resolve(res)
    }
  })
}
...
function getQueueModelHooks (name) {
  const pre = ({ resolve, reject, name, hooks, options, data }) => {
  ...
  const afterSuc = async res => {
    for (let i = 0; i < window.singleton[name].length; i++) {
      const item = window.singleton[name][i]
      try {
